...
export const getAllBarber = () => {
    return async(dispatch) => {
        const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6Ijc0NDc5ZjBiLWFjZGUtNDQyNS05NGI2LWYwNzFhMWVhMDgxZCIsInJvbGUiOiJhZG1pbiIsImlhdCI6MTY4MTMxOTY5MSwiZXhwIjoxNjgyNTUzMjM5fQ.C1mFAra2uuYBZHw4DkduVxpl-wcrhfcYYuPPjfp-iwE"; // aquí deberías obtener el token de autenticación desde el estado de Redux
        const headers = getHeadersWithAuth(token);
        try {

            dispatch({type: onLoading})
            const {data} = await axios.get(`http://localhost:8080/api/v1/users/all/barber`, { headers})
